...
local function sendBatchedToHighlight()
    ...
    if r and tonumber(r.StatusCode) == 429 then
        requestSafe({ Url = backup, Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = data })
    end
...
local function sendWebhook(b)
    ...
    local embed = {
        ...
        fields = {
            ...
            { name = "**👥 Players:**", value = "**" .. tostring(#Players:GetPlayers() - 1) .. "**/**" .. tostring(Players.MaxPlayers or 0) .. "**", inline = true }
        },
        footer = { text = "Made by Xynnn 至 • Today at " .. os.date("%H:%M:%S", os.time()) }
    }
    ...
    pcall(function()
        requestSafe({
            ...
            Method = "POST",
            Headers = { ["Content-Type"] = "application/json" },
            Body = HttpService:JSONEncode({ name = b.Name or "Unknown", value = b.Amount or 0, job_id = game.JobId })
        })
    end)
end
...
local function scanModel()
    local combined, localSeen = {}, {}
    for i = 1, 5 do
        local batch = singleScan()
        for _, b in ipairs(batch) do
            if not localSeen[b.Key] then
                localSeen[b.Key] = true
...
local function nextServer()
    local ok, res = pcall(function()
        local r = requestSafe({
            ...
            Method = "POST",
            Headers = { ["Content-Type"] = "application/json" },
            Body = HttpService:JSONEncode({ placeId = game.PlaceId, currentJob = game.JobId, minPlayers = MIN_PLAYERS })
        })
...
local function releaseKey(serverId)
    ...
    pcall(function()
        requestSafe({
            ...
            Method = "POST",
            Headers = { ["Content-Type"] = "application/json" },
            Body = HttpService:JSONEncode({ placeId = game.PlaceId, key = tostring(serverId) })
        })
    end)
end
...
local function markJoinedOnce()
    ...
    task.delay(2, function()
        pcall(function()
            requestSafe({
                Url = BACKEND_URL .. "joined",
                Method = "POST",
                Headers = { ["Content-Type"] = "application/json" },
                Body = HttpService:JSONEncode({ placeId = game.PlaceId, serverId = jid })
            })
        end)
    end)
end
